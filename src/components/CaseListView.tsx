...
import { Search, Filter, Eye, ChevronDown, ChevronLeft, ChevronRight, X, AlertTriangle, CalendarOff, FileText } from 'lucide-react';
...
export function CaseListView() {
  ...
  const teams = useMemo(() => [...new Set(cases.map((c) => c.team))].sort(), [cases]);

  const overdueIds = useMemo(() => {
    return new Set(alerts.filter((a) => a.type === 'overdue').map((a) => a.caseId));
  }, [alerts]);

  const caseAlertMap = useMemo(() => {
    const m = new Map<string, number>();
    alerts.forEach((a) => m.set(a.caseId, (m.get(a.caseId) || 0) + 1));
    return m;
  }, [alerts]);

  const activeFilterCount = [filterPriority, filterSeverity, filterSituation, filterTeam, filterOverdueOnly, filterNoDeadline].filter(Boolean).length;

  const filtered = useMemo(() => {
    return cases.filter((c) => {
      const q = search.toLowerCase();
      const matchesSearch = !q || c.ppe.toLowerCase().includes(q) || c.victim.toLowerCase().includes(q) || c.suspect.toLowerCase().includes(q);
      ...
      const matchesOverdue = !filterOverdueOnly || overdueIds.has(c.id);
      const matchesNoDeadline = !filterNoDeadline || !c.deadline;
      return matchesSearch && matchesPriority && matchesSeverity && matchesSituation && matchesTeam && matchesOverdue && matchesNoDeadline;
    });
  }, [cases, search, filterPriority, filterSeverity, filterSituation, filterTeam, filterOverdueOnly, filterNoDeadline, overdueIds]);
  ...
  const clearFilters = () => {
    setFilterPriority(''); setFilterSeverity(''); setFilterSituation(''); setFilterTeam('');
    setFilterOverdueOnly(false); setFilterNoDeadline(false); setPage(1);
  };
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/20">
              ...
              <th className="px-4 py-3.5 text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground w-10"></th>
            </tr>
          </thead>
          <tbody>
            {paginated.map((c) => {
                ...
                <tr key={c.id} className={`data-table-row ${isOverdue ? 'bg-destructive/[0.03]' : ''}`}>
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-2">
                      ...
                      {!c.deadline && c.situation !== 'Relatado' && c.situation !== 'Arquivado' && <CalendarOff className="h-3 w-3 text-chart-5" />}
                  ...
                  <td className="px-4 py-3.5 text-xs text-muted-foreground">{c.team.split(' - ')[0]}</td>
